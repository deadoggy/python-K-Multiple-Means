...
class WeightedKMultipleMeans:
    '''

        Simple weighted version of KMM, replace d_{ij}=|x_i-a_j|^2 with d_{ij}=w_i|x_i-a_j|^2.
        Modified lines are tagged with # modified for weight
        ref: 
    ...
    def fit(self, X, W): # modified for weight
        '''
            ...
            argv:
                ...
                @W:
                    np.ndarray, shape=(N_datapoints, ), floats, weight of each point
            return:
        ...
        W /= np.sum(W) # modified for weight
        ...
        while itr_1<self.max_itr_1:
            ...
            for i in range(X.shape[0]):
                dist_to_proto[i] = np.array([W[i] * self.metric(X[i], A[j]) for j in range(A.shape[0])]) # modified for weight
            ...
            for i in range(self.n_proto):
                A[i] = (W*X.T).dot(S[:,i])/np.sum(S[:,i]) # modified for weight
